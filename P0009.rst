...
The current array declaration specification in **8.3.4.p3** restricts
array declarations such that only the leading dimension may be implicit.
...
Note that the following concise and intuitive syntax would require
`relaxation of the current array declaration specification`_
in **8.3.4.p3**.
...
memory architecture, and user access pattern hints.
  ...
  |
  |    template< typename IntegralType >
  |    reference operator[]( IntegralType i ) const noexcept ;
  |
  |    //--------------------
  ...
  |  namespace array_property {
  |
  |  constexpr unsigned maximum_rank = /\* implementation defined \*/ ;
  |
...
When a dimension specification is part of the ``DataType``
the specification is limited by the valid array object declaration
syntax; even though an array object is not being declared.
If a `relaxation of the current array declaration specification`_
were made the ``array_property::dimension`` would be unecessary and
...
..  _`relaxation of the current array declaration specification`:

**Requires slight language specification change and (typically) trivial compiler change for correction and relaxation of array declaration.**
...
Relaxing the **8.3.4.p3** constraint as follows will
  ...
  - allow the proposed syntax for an array_ref with multiple implicit dimensions, and

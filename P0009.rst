...
with polymorphic layout is required
...
memory architecture, and user access pattern hints.
  ...
  |    static constexpr size_t span( IntegralArgs ... implicit_dimensions );
  |
...
.. code-block: c++
  ...
  if ( std::is_lvalue_reference_v< a_type::reference > && a_type::is_regular && 0 <= i && i <= a_type::rank() && 0 <= ji && ji < a.extent(i) - 1 ) {
...
**array_property::layout_order< unsigned ... order >**
  ...
  Requires: The members of ``order`` are [ 0 .. ``sizeof...(order)`` ).
...
.. code-block: c++
  ...
  using a_type = array_ref<int[][3][4],view_property::layout_order<i0,i1,i2> > ;

...
memory architecture, and user access pattern hints.
  ...
  |    constexpr array_ref( const array_ref< UType , UProperties ... > & ) noexcept ;
  |
  |    template< typename UType , typename ... UProperties >
  |    array_ref & operator = ( const array_ref< UType , UProperties ... > & ) noexcept ;
  |  };
  |
  |  }}
  |
...
.. code-block:: c++
  ...
  typename std::enable_if<( 0 <= sizeof...(IntegralArgs) && 1 == rank() )
                         , reference >::type
  operator()( iType0 i0 , IntegralArgs ... indices ) const noexcept ;

  template< typename iType0 , typename iType1 , class ... IntegralArgs >
  typename std::enable_if<( 0 <= sizeof...(IntegralArgs) && 2 == rank() )
                         , reference >::type
  operator()( iType0 i0 , iType1 i1 , IntegralArgs ... indices ) const noexcept ;
...
Subview types are generated with a meta-function.
  ...
  | subarray( const array_ref< DataType, Parameters ... > & , Indices_And_Ranges ... ) noexcept;
  |
  | template< typename T >

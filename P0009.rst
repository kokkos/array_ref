...
**static constexpr unsigned  rank**
  ...
  Declares the rank of the multidimensional array.  Conformal to std::rank< DataType >::value.
...
.. code-block:: c++
  ...
  template < typename A >
  struct array_traits {
    static constexpr size_t rank = std::rank<A>::value ;
...
.. code-block:: c++
  ...
  view< int[][][3] > x(ptr,N0,N1);             // preferred compact notation
  view< int , dimension<0,0,3> > y(ptr,N0,N1); // verbose "magic value" notation
  ...
  assert( x.extent_1() == N0 );
  assert( x.extent_2() == 3 );

  assert( y.extent_0() == N0 );
  assert( y.extent_1() == N0 );
  assert( y.extent_2() == 3 );
...
.. code-block:: c++
  ...
  view< int , dimension<0> > y ; // property syntax
...
.. code-block:: c++
  ...
  struct view {
    ...
    // One plus distance from first member to last member
    constexpr size_t span() const ;

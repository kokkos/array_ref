...
.. code-block:: c++
  ...
  struct array_ref {
    ...
    explicit constexpr array_ref(pointer p, UnsignedIntegers... implicit_dims) noexcept;
    ...
    template <typename... UnsignedIntegers>
    reference operator()(UnsignedIntegers... indices) const noexcept;
  ...
  struct layout_right;
  struct layout_left;
  template <size_t... Ordering> layout_order;
  struct layout_stride;
  ...
  struct dimension {
    ...
    constexpr dimension(DynamicDims... ddims) noexcept;
    ...
    constexpr value_type operator[](size_type dim) const noexcept;
...
.. code-block:: c++
  ...
  using a_type = array_ref<ValueType,Properties...> ;
...
through the ``ValueType`` template argument or through one of the
...
..
  ...
  Remark: When a dimension specification is part of the ``ValueType``
...
**using value_type = typename std::remove_all_extents< ValueType >::type**
...
**constexpr size_type extent( IntegralType index ) const noexcept**
  ...
  Returns: When ``r < rank()`` the extent of dimension, otherwise 0.
...
The 'std::is_assignable' meta-function must be partial specialized to implement
the array_ref assignability rules regarding value type, dimensions, and
properties.

Assignability extends beyond the **cv** qualification of the array_ref's data.  
For example,

  1. The underlying pointer types of the two array_ref<>s must be assignable.
  2. The rank of the two array_ref<>s must be equal.
  3. For each explicit dimension in the LHS array_ref<>, the corresponding
     dimension in the RHS array_ref<> must be an explicit dimension with the
     same extent, or an implicit dimension.

Properties may impose additional assignability conditions, which will be
addressed further in future drafts that describe the property interfaces more
fully.
...
.. code-block:: c++
  ...
  subarray( const array_ref< ValueType, Properties ... > & ar ,
            SliceSpecifiers ... specs) noexcept;
  
  template< typename ValueType , typename ... Properties ,
            typename ... SliceSpecifiers >
  /*unspecified array_property::dimension<>*/ 
  subdimensions( const array_ref< ValueType, Properties ... > & ar ,
                 SliceSpecifiers ... specs) noexcept;
  
  template< typename ValueType , typename ... Properties ,
            typename ... StrideSpecifiers >
  /*unspecified array_ref<>*/ 
  stridearray( const array_ref< ValueType, Properties ... > & ar ,
...
**subarray( const array_ref< T, Properties ... > & ar , SliceSpecifiers ... specs) noexcept**
  ...
  pair<T, T>, a tuple<T, T>, or an array<T, 2>.
